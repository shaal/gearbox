//! Ed25519 verification of a signed catalog over its JCS canonicalization (protocol §7).
//!
//! This is the producer-independent counterpart to the device-side verifier (seed B4):
//! same algorithm, same test vector. The signing input is the catalog with its own
//! `signature` member removed, then JCS-canonicalized.
