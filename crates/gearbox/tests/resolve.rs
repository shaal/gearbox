...
fn store(id: &str, priority: u32, enabled: bool) -> StoreRef {
    StoreRef { id: id.into(), priority, enabled }
}
...
fn pins(pairs: &[(&str, &str)]) -> HashMap<String, String> {
    pairs.iter().map(|(c, s)| ((*c).into(), (*s).into())).collect()
}
...
fn parse_ref_variants() {
    assert_eq!(resolve::parse_ref("doom").unwrap(), CogRef::Bare { cog_id: "doom".into() });
    assert_eq!(
        resolve::parse_ref("acme/doom").unwrap(),
        CogRef::Namespaced { store_id: "acme".into(), cog_id: "doom".into() }
    );
...
fn new_rejects_bad_inputs() {
    assert!(Resolver::new(vec![store("a", 0, true), store("a", 1, true)], HashMap::new(), HashMap::new()).is_err());
    assert!(Resolver::new(vec![store("a", 0, true)], offerings(&[("ghost", &["x"])]), HashMap::new()).is_err());
    assert!(Resolver::new(vec![store("a", 0, true)], HashMap::new(), pins(&[("x", "ghost")])).is_err());
    assert!(Resolver::new(vec![store("a", 0, true)], offerings(&[("a", &["x"])]), pins(&[("x", "a")])).is_ok());
}
...
fn namespaced_resolution() {
    let r = Resolver::new(
        vec![store("official", 0, true), store("acme", 10, true), store("off", 5, false)],
        offerings(&[("official", &["doom"]), ("acme", &["doom"]), ("off", &["doom"])]),
        HashMap::new(),
    )
    .unwrap();

    let res = r.resolve("acme/doom").unwrap();
    assert_eq!((res.store_id.as_str(), res.cog_id.as_str(), res.reason), ("acme", "doom", ResolveReason::Explicit));

    assert_eq!(r.resolve("nope/doom"), Err(ResolveError::UnknownStore("nope".into())));
    assert_eq!(r.resolve("off/doom"), Err(ResolveError::StoreDisabled("off".into())));
    assert_eq!(r.resolve("official/ghost"),
        Err(ResolveError::NotOffered { store_id: "official".into(), cog_id: "ghost".into() }));
}
...
fn bare_resolves_by_priority_then_id() {
    let r = Resolver::new(
        vec![store("official", 0, true), store("acme", 10, true), store("beta", 0, true)],
        offerings(&[("official", &["doom"]), ("acme", &["doom"]), ("beta", &["doom"])]),
        HashMap::new(),
    )
    .unwrap();
    ...
    assert_eq!((res.store_id.as_str(), res.reason), ("beta", ResolveReason::Priority));
...
fn bare_skips_disabled_and_non_offering() {
    let r = Resolver::new(
        vec![store("a", 0, false), store("b", 5, true), store("c", 1, true)],
    ...
    assert_eq!(r.resolve("missing"), Err(ResolveError::NotFound("missing".into())));
}
...
fn pin_overrides_priority() {
    ...
    .unwrap();
    let res = r.resolve("doom").unwrap();
    assert_eq!((res.store_id.as_str(), res.reason), ("acme", ResolveReason::Pinned));
...
fn pin_to_unusable_store_errors() {
    ...
    .unwrap();
    assert_eq!(disabled.resolve("doom"), Err(ResolveError::StoreDisabled("acme".into())));
    ...
    .unwrap();
    assert_eq!(not_offered.resolve("doom"),
        Err(ResolveError::NotOffered { store_id: "acme".into(), cog_id: "doom".into() }));
}
...
fn collisions_lists_shared_cogs_among_enabled_stores() {
    let r = Resolver::new(
        vec![store("a", 0, true), store("b", 1, true), store("c", 2, false)],
        offerings(&[("a", &["doom", "solo"]), ("b", &["doom"]), ("c", &["doom"])]),
        HashMap::new(),
    )
    .unwrap();
    // c is disabled, so doom collides only across a + b; solo is unique
    assert_eq!(r.collisions(), vec![("doom".to_string(), vec!["a".to_string(), "b".to_string()])]);
}
...
fn cog_ids_extracts_from_catalog() {
    ...
    assert_eq!(gearbox::catalog::cog_ids(&catalog), vec!["doom", "adversarial"]);
}
